...
class BlockChain(Node):
    ...
    def new_block(self):
        ...
        block = Block(
            len(self.blocks), time.time(), self.current_transactions, previous_hash
        )
    ...
    def node_message(self, connected_node, message):
        ...
        if len(blocks) > len(self.blocks):
            ...
            self.current_transactions = []
            print(
                "Received chain is bigger than currrent one. Current one will be replaced by received one"
            )
            return True
        else:
            print(
                "Received chain is not bigger than current one. Current one will be kept"
            )
            return False
    ...
    def repr_json(self):
        return {
            ...
            "current_transactions": [
                transaction.repr_json() for transaction in self.current_transactions
            ],
        }
...
class Block(object):
    def __init__(
        self, index, timestamp, transactions, previous_hash, proof=None, *args, **kwargs
    ):
        self.index = index
    ...
    def valid_proof(self, proof, difficulty):
        hash_funct = hashlib.sha256(
            (
                str(self.index)
                + str(self.timestamp)
                + "".join(list(map(lambda x: x.hash, self.transactions)))
